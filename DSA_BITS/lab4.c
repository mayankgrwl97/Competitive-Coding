/*Take care for AAAAA type case in the function huff_code*/
/*Take care of order n which frequencies as mentioned in point 1*/

//This code is in alphabetical order for frequencies
...
int main()
{
	...
	for(i=0; i<26; i++)
	{
		if(freq[i] != 0)
		{
			ptr[i] = min_queue.arr[j++] = get_node(i+'A', freq[i]);
			min_queue.length++;
		}
	}
...
void min_heapify(heap* A, int i)
{
	...
	if(l<= A->heap_size && A->arr[l]->freq < A->arr[i]->freq)
		min = l;

	if(r<= A->heap_size && A->arr[r]->freq < A->arr[min]->freq)
		min = r;
